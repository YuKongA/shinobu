...
mod platform {
    ...
    #[cfg(test)]
    mod tests {
        use super::parse_proc_status_bytes;

        #[test]
        fn parses_proc_status_kib_fields() {
            let status = "Name:\ttest\nVmSize:\t  123 kB\nVmRSS:\t  45 kB\n";

            assert_eq!(parse_proc_status_bytes(status, "VmSize:"), Some(125_952));
            assert_eq!(parse_proc_status_bytes(status, "VmRSS:"), Some(46_080));
        }
    }
}
...
mod tests {
    ...
    fn uptime_clock_reports_elapsed_time() {
        let clock =
            UptimeClock::with_started_at(std::time::Instant::now() - Duration::from_secs(5));
